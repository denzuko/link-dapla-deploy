Feature: link-dapla-deploy rootless Podman quadlet deploy
  ...
  Scenario: Network unit uses netavark bridge
    When link-network-sections is called
  ...
  Scenario: Network unit has correct VLSM allocation
    When link-network-sections is called
    Then the INI contains "Subnet=10.89.2.32/30"
    And the INI contains "Gateway=10.89.2.33"

  Scenario: Container home profile is read-only via %%h
    When link-container-sections is called
  ...
  Scenario: HAProxy backend targets netavark gateway
    ...
    Then the output contains "10.89.2.33:4567"
