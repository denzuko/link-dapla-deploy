...
(def-suite :quadlet-specifiers
  :description "Quadlet specifier correctness for link.dapla.net.")

(in-suite :quadlet-specifiers)

(defun network-ini () (cinix-write-string (link-network-sections)))
(defun container-ini () (cinix-write-string (link-container-sections)))
...
(test network-vlsm
  "Network unit has correct VLSM subnet 10.89.2.32/30 and gateway 10.89.2.33."
  (let ((ini (network-ini)))
...
(test haproxy-netavark-gateway
  "HAProxy backend uses netavark gateway 10.89.2.33:4567, not loopback."
...
(defun run-spec ()
  (let ((results (run :quadlet-specifiers)))
    ...
    (unless (every #'fiveam::test-passed-p results)
      (error "link-dapla-deploy spec suite: tests failed."))))
