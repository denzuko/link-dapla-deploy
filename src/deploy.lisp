...
;;;;
;;;; Consfigurator properties and DEFHOST for Chhoto URL shortener at link.dapla.net.
...
(defparameter *users-chhoto-dataset* "storage/users/chhoto")
(defparameter *users-chhoto-mountpoint* "/var/lib/chhoto"
  "Service account home.")
(defparameter *users-chhoto-dataset-keyfile* "/etc/zfs-keys/chhoto-users.key")

(defparameter *containers-chhoto-dataset* "storage/containers/chhoto")
(defparameter *containers-chhoto-mountpoint* "/srv/chhoto"
  "Chhoto SQLite database.")
(defparameter *containers-chhoto-dataset-keyfile* "/etc/zfs-keys/chhoto-data.key")
...
(defun link-network-sections ()
  "Cinix AST for link.network: netavark bridge on podman9 (10.89.2.32/30)."
  '(("Network" . (("NetworkName" . "link")
                   ("Driver"      . "bridge")
                   ("Subnet"      . "10.89.2.32/30")
                   ("Gateway"     . "10.89.2.33")))))

(defun link-container-sections ()
  "Cinix AST for link.container. HAProxy backend: 10.89.2.33:4567."
  `(("Unit" . (("Description" . "Chhoto URL shortener")))
    ("Container" . (("Image"         . "oci.dapla.net/sintan1729/chhoto-url:latest")
                    ...
                    ("Volume" . "%h:/var/lib/link:ro")
                    ("Volume" . "/srv/%U/data:/app/urls.sqlite:Z")
...
(defun haproxy-vhost-config ()
  "HAProxy vhost configuration for link.dapla.net.
   Backend: 10.89.2.33:4567 (netavark bridge podman9, subnet 10.89.2.32/30)."
  (format nil
"frontend link_http
  bind *:80
...
(defprop quadlets-written :posix (user home)
  "Write all link quadlet unit files into USER's systemd container directory."
  (:desc (format nil "Chhoto URL shortener quadlet units written for ~A" user))
  (:apply
   (let ((quadlet-dir (format nil "~A/.config/containers/systemd" home)))
     (containing-directory-exists (format nil "~A/link.network" quadlet-dir))
     (write-remote-file (format nil "~A/link.network" quadlet-dir)
                        (cinix-write-string (link-network-sections)))
     (write-remote-file (format nil "~A/link.container" quadlet-dir)
                        (cinix-write-string (link-container-sections))))))

(defprop quadlets-activated :posix (user)
  "Reload USER's user-scope systemd daemon and restart link services."
...
(defprop haproxy-vhost-written :posix ()
  "Write the HAProxy vhost config for link.dapla.net. Reloads HAProxy when content changes."
...
(defhost link-host (:deploy (:local))
  "The Chhoto URL shortener host."
  (zfs-encryption-key *users-chhoto-dataset-keyfile*)
  (zfs-encryption-key *containers-chhoto-dataset-keyfile*)
  (zfs-dataset-mounted *users-chhoto-dataset* *users-chhoto-mountpoint* *users-chhoto-dataset-keyfile*)
  (zfs-dataset-mounted *containers-chhoto-dataset* *containers-chhoto-mountpoint* *containers-chhoto-dataset-keyfile*)
  (rootless-service-account *service-user* *users-chhoto-mountpoint*)
  (lingering-enabled *service-user*)
  (images-pulled *service-user*
                 "oci.dapla.net/sintan1729/chhoto-url:latest")
  (quadlets-written *service-user* *users-chhoto-mountpoint* *containers-chhoto-mountpoint*)
...
(defun deploy-app ()
  "Provision Chhoto URL shortener via LINK-HOST (Consfigurator, :local connection).
  ...
  (format t "~&--> Chhoto URL shortener provisioned. Visit https://~A~%" *haproxy-fqdn*))
